...
from hyper.utils import hyperboloid_to_poincare_ball, load_data, load_embedding
from hyper.utils import perform_walks, determine_positive_and_negative_samples
from hyper.losses import  hyperbolic_softmax_loss, hyperbolic_sigmoid_loss, hyperbolic_hypersphere_loss
from hyper.generators import TrainingDataGenerator
from hyper.visualise import draw_graph, plot_degree_dist
from hyper.callbacks import Checkpointer
...
def parse_args():
	...
	parser.add_argument('--visualise', action="store_true", default=True, 
...
def main():
	...
	if args.visualise and args.embedding_dim == 2:
		...
		draw_graph(graph, 
			embedding, 
			node_labels, 
			path=str(args.alpha)+"2d-poincare-disk-visualisation.png")
